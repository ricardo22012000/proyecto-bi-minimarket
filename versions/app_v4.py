"""
============================================================
DASHBOARD BI - MINIMARKET - VERSIÓN 4
============================================================
Autor: Ricardo Muñoz
...
MEJORAS RESPECTO A V3:
  ✅ KPIs con títulos claramente visibles (fix modo oscuro)
  ✅ Gráfico de evolución tipo "olas" (spline suave)
  ✅ Marcadores destacando picos máximos
  ✅ Anotaciones en máximos de Ventas y Ganancia
...
# ============================================================
st.set_page_config(
    page_title="Dashboard BI v4 - Minimarket | G&S",
    page_icon="🛒",
    layout="wide",
    initial_sidebar_state="expanded"
)

# CSS mejorado con KPIs visibles en modo claro y oscuro
st.markdown("""
    <style>
    .block-container {
        padding-top: 1rem;
...
# ============================================================
# FUNCIÓN: KPIs CON TÍTULOS VISIBLES (HTML custom)
# ============================================================
def kpi_card(icono, titulo, valor):
    """Genera una tarjeta KPI con título visible en cualquier tema."""
    return f"""
    <div class="kpi-card">
...
if pagina == "📊 General":
    ...
    # === GRÁFICO DE OLAS (spline con picos destacados) ===
    ...
    # Identificar los picos máximos
    idx_max_ventas = ventas_mensuales["TotalVenta"].idxmax()
    idx_max_ganancia = ventas_mensuales["Ganancia"].idxmax()

    pico_ventas_x = ventas_mensuales.loc[idx_max_ventas, "AñoMes"]
    pico_ventas_y = ventas_mensuales.loc[idx_max_ventas, "TotalVenta"]
    pico_ganancia_x = ventas_mensuales.loc[idx_max_ganancia, "AñoMes"]
    pico_ganancia_y = ventas_mensuales.loc[idx_max_ganancia, "Ganancia"]

    fig_area = go.Figure()

    # Onda de Ventas (spline azul)
    ...
    # Marcador en pico máximo de Ventas
    fig_area.add_trace(go.Scatter(
        x=[pico_ventas_x], y=[pico_ventas_y],
        mode="markers+text",
        marker=dict(color="#1f4e79", size=14, symbol="star",
                    line=dict(color="white", width=2)),
        text=[f"🔝 ${pico_ventas_y:,.0f}"],
        textposition="top center",
        textfont=dict(color="#1f4e79", size=11, family="Arial Black"),
        showlegend=False,
        hovertemplate=f"<b>Pico Ventas</b><br>{pico_ventas_x}<br>${pico_ventas_y:,.0f}<extra></extra>"
    ))

    # Marcador en pico máximo de Ganancia
    fig_area.add_trace(go.Scatter(
        x=[pico_ganancia_x], y=[pico_ganancia_y],
        mode="markers+text",
        marker=dict(color="#28a745", size=14, symbol="star",
                    line=dict(color="white", width=2)),
        text=[f"🔝 ${pico_ganancia_y:,.0f}"],
        textposition="bottom center",
        textfont=dict(color="#28a745", size=11, family="Arial Black"),
        showlegend=False,
        hovertemplate=f"<b>Pico Ganancia</b><br>{pico_ganancia_x}<br>${pico_ganancia_y:,.0f}<extra></extra>"
    ))
