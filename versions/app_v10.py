"""
============================================================
DASHBOARD BI - MINIMARKET - VERSIÓN 10
============================================================
Autor: Ricardo Muñoz
Empresa: G&S Gestión y Sistemas
Fecha: Julio 2026
============================================================
MEJORAS RESPECTO A V9:
  ✅ Header simplificado: solo muestra fecha/hora de actualización
  ✅ Ventas por Sucursal ahora incluye % del total
  ✅ Ventas por Categoría: la categoría con menor % en rojo (crítico)
  ✅ Fix menor en el hover de "Ventas por Día"
============================================================
"""
...
# ============================================================
st.set_page_config(
    page_title="Dashboard BI v10 - Minimarket | G&S",
    page_icon="🛒",
    layout="wide",
...
def obtener_fecha_actualizacion():
    """Lee la fecha/hora limpia desde el log (sin prefijos)."""
    if RUTA_LOG.exists():
        try:
...
st.sidebar.caption("G&S • v10 alertas críticas")
...
st.markdown(
    f'<p class="sub-header">🕓 Actualizado: {fecha_upd}</p>',
    unsafe_allow_html=True
)
...
if pagina == "📊 General":
    ...
    if len(ventas_periodo) < 2:
        ...
        fig_bar.add_trace(go.Bar(
            x=ventas_periodo[col_periodo], y=ventas_periodo["TotalVenta"],
            name="Ventas", marker_color="#1f4e79",
            text=ventas_periodo["TotalVenta"].apply(lambda x: f"${x:,.0f}"), textposition="outside"
        ))
        fig_bar.add_trace(go.Bar(
            x=ventas_periodo[col_periodo], y=ventas_periodo["Ganancia"],
    ...
    # ============================================================
    # 🏪 VENTAS POR SUCURSAL con % ⭐ CAMBIO
    # ============================================================
    with col1:
        ...
        # Texto combinado: $XXXk (XX.X%)
        vs["Etiqueta"] = vs.apply(
            lambda r: f"${r['TotalVenta']/1000:.0f}K ({r['Porcentaje']:.1f}%)", axis=1
        )
        ...
        fig_vs.update_layout(height=230, margin=dict(l=10, r=10, t=10, b=10),
                              showlegend=False, xaxis_title="", yaxis_title="")
        st.plotly_chart(fig_vs, use_container_width=True)

    # ============================================================
    # 📦 VENTAS POR CATEGORÍA (con alerta roja al menor) ⭐ CAMBIO
    # ============================================================
    with col2:
        ...
        # Identificar la categoría con menor venta (crítica)
        idx_min = vc["TotalVenta"].idxmin()
        categoria_critica = vc.loc[idx_min, "Categoría"]

        # Paleta base de azules para las demás + rojo para la crítica
        paleta_base = ["#1f4e79", "#2E6BA8", "#4A90E2", "#6BA5D9", "#8FBCE0", "#B3D3E6"]
        colores_cat = []
        for i, cat in enumerate(vc["Categoría"]):
            if cat == categoria_critica:
                colores_cat.append("#E74C3C")  # 🔴 Rojo crítico
            else:
                # Asignar color de la paleta (respetando el orden)
                idx_paleta = i % len(paleta_base)
                colores_cat.append(paleta_base[idx_paleta])

        fig_vc = go.Figure(data=[go.Pie(
            labels=vc["Categoría"], values=vc["TotalVenta"], hole=0.55,
        ...
        fig_vc.update_layout(
            ...
            annotations=[dict(
                text=f"🔴 Crítico:<br><b>{categoria_critica[:15]}{'...' if len(categoria_critica) > 15 else ''}</b>",
...
# ============================================================
    ...
    # 📅 VENTAS POR DÍA (%) con colores vibrantes (fix hover)
    with col2:
        ...
        colores_dia = {
            "Lunes": "#1f4e79", "Martes": "#1f4e79",
            "Miércoles": "#1f4e79", "Jueves": "#1f4e79",
            "Viernes": "#FF7F50", "Sábado": "#E74C3C", "Domingo": "#9B59B6"
        }
        colores_barras = [colores_dia.get(str(d), "#1f4e79") for d in vd["DiaSemana"]]

        fig_vd = go.Figure()
        fig_vd.add_trace(go.Bar(
            x=vd["DiaAbrev"], y=vd["Porcentaje"],
