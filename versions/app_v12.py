"""
============================================================
DASHBOARD BI - MINIMARKET - VERSIÓN 12
============================================================
Autor: Ricardo Muñoz
Empresa: G&S Gestión y Sistemas
Fecha: Julio 2026
============================================================
MEJORAS RESPECTO A V11:
  ✅ Eliminada la predicción de stock (no había datos reales)
  ✅ Predicción de ganancia enriquecida
  ✅ Predicción adicional de ventas totales
  ✅ Métricas técnicas del modelo (R², MAE, tendencia)
  ✅ Insights automáticos legibles para gerencia
============================================================
"""
...
# ============================================================
st.set_page_config(
    page_title="Dashboard BI v12 - Minimarket | G&S",
    page_icon="🛒",
    layout="wide",
...
st.sidebar.caption("G&S • v12 predicción enriquecida")
...
# ============================================================
# 🔮 PÁGINA 4: PREDICCIONES (ENRIQUECIDA)
# ============================================================
    ...
    if pred_ganancia is None:
        ...
        # === FILA 2: GRÁFICO PRINCIPAL + MÉTRICAS TÉCNICAS ===
        ...
        with col2:
            st.markdown('<p class="section-title">🔬 Métricas técnicas</p>', unsafe_allow_html=True)

            # Tabla de métricas
            metricas_df = pd.DataFrame({
                "Métrica": ["R² Score", "MAE", "Tendencia", "Desv. estándar", "Prom. histórico", "Mín histórico", "Máx histórico"],
                "Valor": [
                    f"{pred_ganancia['r2']*100:.1f}%",
                    f"${pred_ganancia['mae']:,.0f}",
                    f"${pred_ganancia['coef_tendencia']:+,.0f}/mes",
                    f"${pred_ganancia['std_dev']:,.0f}",
                    f"${pred_ganancia['promedio_historico']:,.0f}",
                    f"${df_hist['Valor'].min():,.0f}",
                    f"${df_hist['Valor'].max():,.0f}"
                ]
            })
            st.dataframe(metricas_df, use_container_width=True, hide_index=True, height=290)
